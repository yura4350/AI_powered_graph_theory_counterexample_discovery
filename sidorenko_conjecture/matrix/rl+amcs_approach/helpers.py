...
def calculate_gap(M):
    """
    Calculates a score based on the eigenvalue formulation of Sidorenko's
    conjecture for the graph H = K5,5 \ C10.
    """
...
# Function to preturb the matrix before the RL run
def strong_perturbation(matrix, strength=0.2):
    noise = np.random.normal(loc=0.0, scale=strength, size=matrix.shape)
    perturbed_matrix = matrix + noise
    return np.clip(perturbed_matrix, 0, 1)