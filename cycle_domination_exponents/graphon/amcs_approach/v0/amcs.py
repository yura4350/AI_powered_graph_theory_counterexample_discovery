...
def NMCS(H1, H2, current_W, steps, score_function):
    """
    Local search (exploration phase) for the cycle exponent problem.
    """
    best_W_local = current_W.copy()
    best_score_local = score_function(best_W_local, H1, H2)
...
def AMCS(H1, H2, initial_W, max_depth=5, max_level=3):
    ...
    while level <= max_level:
        ...
        next_W = NMCS(H1, H2, current_W, steps=nmcs_steps, score_function=score_function)
